...
fn drop_impl_extra_where_clause() {
    ...
    .err(expect_test::expect![[r#"
        crates/formality-rust/src/prove/prove_via.rs:9:1: no applicable rules for prove_via { goal: Clone(!ty_0), via: @ coinductive(Drop(MyStruct<!ty_0>)), assumptions: {@ coinductive(Drop(MyStruct<!ty_0>))}, env: Env { variables: [!ty_0], bias: Soundness, pending: [], allow_pending_outlives: false } }
...
fn drop_impl_enum_extra_where_clause() {
    ...
    .err(expect_test::expect![[r#"
        crates/formality-rust/src/prove/prove_via.rs:9:1: no applicable rules for prove_via { goal: Clone(!ty_0), via: @ coinductive(Drop(MyEnum<!ty_0>)), assumptions: {@ coinductive(Drop(MyEnum<!ty_0>))}, env: Env { variables: [!ty_0], bias: Soundness, pending: [], allow_pending_outlives: false } }
