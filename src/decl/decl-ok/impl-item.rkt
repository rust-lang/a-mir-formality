...
(define-metafunction formality-decl
  ...
  [(impl-item-ok-goal CrateDecls TraitRef_impl FnDecl)
   ...
   (where/error (fn FnId (KindedVarId ...) (Ty_arg ...) -> Ty_ret where [Biformula ...] _) FnDecl)
   ...
   (where/error (_ ... (fn FnId KindedVarIds_trait-fn (Ty_trait-fn-arg ...) -> Ty_trait-fn-ret where Biformulas_trait-fn _) _ ...) TraitItems_trait)
   ...
   (where/error Goal_implies
                (∀ [KindedVarId ...]
                   (implies
                    (apply-substitution Substitution_trait->impl Biformulas_trait-fn)
  ...
  [; For an associated type value
   ...
   ; unpack things
   (where/error (TraitId (Parameter_trait ...)) TraitRef_impl)
   (where/error (type AssociatedTyId (KindedVarId ...) = Ty where Biformulas) AssociatedTyValue)

   ; find the declaration of this associated type
   (where/error (trait TraitId KindedVarIds_trait where Biformulas_trait TraitItems_trait) (trait-with-id CrateDecls TraitId))
   (where/error (_ ... (type AssociatedTyId KindedVarIds_trait-ty BoundsClause_trait-ty where Biformulas_trait-ty) _ ...) TraitItems_trait)
   (where/error ((_ VarId_trait) ...) KindedVarIds_trait)
   ...
   (where/error [Biformula_bty ...] (instantiate-bounds-clause BoundsClause_trait-ty Ty))
   ...
   (where/error Goal_ty-wf-and-meets-bounds
                (implies Biformulas
                         (&& (Biformula_bty ... ; (a) Ty meets bounds declared in trait
   ...
   (where/error Goal_implies
                (implies
                 (apply-substitution Substitution_trait->impl Biformulas_trait-ty)
                 (&& Biformulas)
                 ))
   ]
  )
