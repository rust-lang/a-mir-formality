...
fn test_let_with_ill_formed_type() {
    crate::assert_err!(
        ...
        expect_test::expect![[r#"
            crates/formality-rust/src/prove/prove/prove/prove_via.rs:9:1: no applicable rules for prove_via { goal: Trait1(S1), via: HasPlace(?ty_1), assumptions: {}, env: Env { variables: [?ty_1, ?ty_2, ?ty_3], bias: Soundness, pending: [], allow_pending_outlives: true } }

            crates/formality-rust/src/prove/prove/prove/prove_via.rs:9:1: no applicable rules for prove_via { goal: Trait1(S1), via: HasPlace(?ty_1), assumptions: {}, env: Env { variables: [?ty_1, ?ty_2], bias: Soundness, pending: [], allow_pending_outlives: true } }

            crates/formality-rust/src/prove/prove/prove/prove_via.rs:9:1: no applicable rules for prove_via { goal: Trait1(S1), via: HasPlace(?ty_1), assumptions: {}, env: Env { variables: [?ty_1], bias: Soundness, pending: [], allow_pending_outlives: true } }
...
fn test_break_nonexistent_label() {
    crate::assert_err!(
        ...
        expect_test::expect![[r#"
            crates/formality-rust/src/check/borrow_check/nll.rs:167:1: no applicable rules for borrow_check_statement { state: flow_state([scope(none, None, {}, None, [], []), scope(none, None, {}, None, [], []), scope(none, None, {}, Some({}), [], []), scope(none, None, {}, None, [], [])], point_flow_state({}, {}), {}, {}), statement: break 'nonexistent ;, places_live_on_exit: {}, assumptions: {}, env: TypeckEnv { env: Env { variables: [], bias: Soundness, pending: [], allow_pending_outlives: false }, output_ty: Some(u32), program: program([crate core { trait HasPlace <ty> { type Target : [] ; } unsafe trait Projection <ty> { type Source : [] ; type Target : [] ; fn offset (self : ^ty1_0) -> usize ; } unsafe trait PlaceRead <ty, ty> where ^ty0_0 : HasPlace, ^ty0_1 : Projection, <^ty0_1 as Projection>::Source => <^ty0_0 as HasPlace>::Target { unsafe fn read (this : *const ^ty1_0, proj : ^ty1_1) -> <^ty1_1 as Projection>::Target ; } unsafe trait PlaceWrite <ty, ty> where ^ty0_0 : HasPlace, ^ty0_1 : Projection, <^ty0_1 as Projection>::Source => <^ty0_0 as HasPlace>::Target { unsafe fn write (this : *const ^ty1_0, proj : ^ty1_1, value : <^ty1_1 as Projection>::Target) -> () ; } unsafe trait PlaceMove <ty, ty> where ^ty0_0 : PlaceRead <^ty0_1>, ^ty0_1 : Projection, <^ty0_1 as Projection>::Source => <^ty0_0 as HasPlace>::Target { } unsafe trait DropHusk <ty> where ^ty0_0 : HasPlace { unsafe fn drop_husk (this : *const ^ty1_0) -> () ; } unsafe trait PlaceDrop <ty, ty> where ^ty0_0 : HasPlace, ^ty0_1 : Projection, <^ty0_1 as Projection>::Source => <^ty0_0 as HasPlace>::Target { unsafe fn drop (this : *const ^ty1_0, proj : ^ty1_1) -> () ; } unsafe trait PlaceBorrow <ty, ty, ty> where ^ty0_0 : HasPlace, ^ty0_1 : Projection, <^ty0_1 as Projection>::Source => <^ty0_0 as HasPlace>::Target { type BorrowDuration : [BorrowDuration] ; unsafe fn borrow (this : *const ^ty1_0, proj : ^ty1_1) -> ^ty1_2 ; } trait BorrowDuration <ty> { } struct Instant { } struct Lifetime <lt> { } struct Indefinite { } impl BorrowDuration for Instant { } impl <lt> BorrowDuration for Lifetime<^lt0_0> { } impl BorrowDuration for Indefinite { } unsafe trait PlaceWrapper <ty, ty> where ^ty0_0 : HasPlace, ^ty0_1 : Projection, <^ty0_1 as Projection>::Source => <^ty0_0 as HasPlace>::Target { type WrappedProjection : [Projection] where <<^ty1_0 as PlaceWrapper<^ty1_1>>::WrappedProjection as Projection>::Source => ^ty1_0 ; fn wrap_projection (proj : ^ty1_1) -> <^ty1_0 as PlaceWrapper<^ty1_1>>::WrappedProjection ; } unsafe trait PlaceDeref <ty, ty> where ^ty0_0 : HasPlace, ^ty0_1 : Projection, <^ty0_1 as Projection>::Source => <^ty0_0 as HasPlace>::Target, <^ty0_1 as Projection>::Target : HasPlace { unsafe fn deref (ptr : *mut ^ty1_0, proj : ^ty1_1) -> *const <^ty1_1 as Projection>::Target ; } }, crate Foo { fn foo () -> u32 { loop { break 'nonexistent ; } return 0 _ u32 ; } }], 222) } }
