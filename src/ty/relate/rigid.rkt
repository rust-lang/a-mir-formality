...
(define-metafunction formality-ty
  relate-var-to-rigid : Env (VarId RelationOp (rigid-ty RigidName Parameters)) -> (Env Goals)
  ...
  [; X <op> R<...> -- Inequality between a variable and a rigid type
   ...
   (relate-var-to-rigid Env (VarId RelationOp (rigid-ty RigidName (Parameter ...))))
   (Env (Goal))
...
(define-metafunction formality-ty
  relate-rigid-to-rigid : Env ((rigid-ty RigidName Parameters) RelationOp (rigid-ty RigidName Parameters)) -> (Env Goals)

  [; Relating two rigid types with the same name: relate their parameters according to the declared variance.
   (relate-rigid-to-rigid Env ((rigid-ty RigidName (Parameter_1 ..._1)) RelationOp (rigid-ty RigidName (Parameter_2 ..._1))))
