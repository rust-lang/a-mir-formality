...
fn parse_forall_goal() {
    ...
    expect![[r#"
        for_all(<ty> is_implemented(Debug(^ty0_0)))
    "#]]
    .assert_debug_eq(&program);
}
...
fn parse_nested_binders() {
    ...
    expect![[r#"
        for_all(<ty> exists(<ty> is_implemented(Debug(^ty1_0, ^ty0_0))))
    "#]]
    .assert_debug_eq(&program);
}
...
fn parse_all() {
    ...
    expect![[r#"
        for_all(<ty> all(exists(<ty> is_implemented(PartialEq(^ty1_0, ^ty0_0))), has_impl(Debug(^ty0_0))))
    "#]]
    .assert_debug_eq(&program);
}
