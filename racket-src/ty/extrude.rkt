...
(define-metafunction formality-ty
  ...
  [(extrude-term Env VarIdPairs Universe InequalityOp (implies Biformulas Ty))
   (Env_out (implies Biformulas Ty_out) (append Goals_ty Goals_wc))

   (where/error (Env_1 Ty_out Goals_ty) (extrude-term Env VarIdPairs Universe InequalityOp Ty))
   (where/error (Env_2 Biformulas_out Goals_wc) (extrude-terms Env VarIdPairs Universe InequalityOp Biformulas))
   ]
