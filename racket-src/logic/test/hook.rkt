  ...
  [(env-with-clauses-and-invariants Clauses Invariants)
   (env-with-hook (Hook: ,(begin
                            (formality-logic-hook
                             ...
                             (lambda (env goal) (term (logic:categorize-goal ,env ,goal)))
                             ))))
   ]
  )
...
(define-metafunction formality-logic
  ...
  logic:categorize-goal : Env Goal -> Goal/Categorization

  [(logic:categorize-goal _ BuiltinGoal) builtin-goal]

  [(logic:categorize-goal _ (Parameter_1 == Parameter_2)) builtin-relation]
  ...
  [(logic:categorize-goal _ VarId) (user-predicate +)]
  [(logic:categorize-goal _ (VarId Parameters_in -> Parameters_out)) (user-predicate +)]
  [(logic:categorize-goal _ (VarId Parameter ...)) (user-predicate +)]
  )
